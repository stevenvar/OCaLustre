...
let rec seq_exp e p =
  ...
  | Variable s -> S_Variable s
  | Application (i, e) ->
    S_Application (i^"_next", seq_exp e p)
  | Call e ->
    S_Call e
  | InfixOp (op,e1,e2) ->
    S_InfixOp(seq_infop op,
...
let call_state e l name =
  match e.e_desc with
  | Application (i, e) ->
    { s_pattern = Parsing_ocl.mk_pattern (i^"_state_");
      s_expression = S_Application_init (i^"_0", seq_exp e l)}
    ::l
  | _ -> l
...
let state_eq e ({pres;calls;outs} as s) =
  ...
  | Pre e' ->
    begin
      match e'.e_desc with
      | Variable n -> { s with pres = n::calls }
      | _ -> assert false
    end
  | Fby (e,e') ->
    begin
...
let seq_zero name inputs outputs state eqs =
  let call_init =
    List.fold_left
      (fun acc eq -> call_state eq.expression acc (string_of_pattern name)) [] eqs in
  { s_name = name;
    s_inputs = inputs;
    s_outputs = outputs;
    s_state = state;
    s_eqs = call_init @ (seq_eqs_zero eqs) }
