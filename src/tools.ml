...
let rec prefix_pattern ?pre:(pre="st_") p =
  ...
  let new_desc = match p.p_desc with
    ...
    | PUnit -> PUnit
    | Typed (p',_s) -> (prefix_pattern ~pre:pre p').p_desc
  in
...
let rec suffix_pattern ?suf:(suf="st_") p =
  ...
  let new_desc = match p.p_desc with
    ...
    | PUnit -> PUnit
    | Typed (p',_s) -> (suffix_pattern ~suf:suf p').p_desc
  in
  { p with p_desc = new_desc }
...
let rec string_of_pattern ?(prefix="") ?(suffix="") p =
  ...
  | Typed (p,_t) -> string_of_pattern p
  | PUnit -> "()"
  | Tuple pl ->
    "("^aux pl^")"
...
let rec string_list_of_pattern p =
  ...
  match p.p_desc with
  | Ident i -> i::[]
  | Typed (p,_t) -> string_list_of_pattern p
  | PUnit -> ["()"]
  | Tuple tl ->
...
let rec expr_of_pattern p =
  ...
  | Typed (p,_s) -> expr_of_pattern p

let rec pat_of_pattern p =
  let open Parsing_ast in
  match p.p_desc with
  | Ident _i -> { ppat_desc = Ppat_var (stringloc_of_pattern p) ;
                  ppat_loc = p.p_loc ;
                  ppat_attributes = [] ;
                  ppat_loc_stack = [] }
  | Tuple t ->
