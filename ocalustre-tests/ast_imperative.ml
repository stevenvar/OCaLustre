...
let rec compile_expression (exp,clock) =
  ...
  let rec compile_expression_without_clock exp = 
    match exp with
      | C_Value v -> IValue v
      | C_Tuple t ->
        ITuple (List.map (compile_expression_without_clock) (List.map fst t))
      ...
      | C_Application (id, el) ->
        IApplication (
          id, List.map (compile_expression_without_clock) (List.map fst el)
        )
      | C_Application_init (id, el) ->
        IApplication_init (
          id, List.map (compile_expression_without_clock) (List.map fst el)
        )
      | C_Call e -> ICall e 
...
let generate_app_inits node =
  let generate_from_e equation l =
    match fst equation.c_expression with
    | C_Application_init (i, li) ->
      (
        equation.c_pattern,
        IApplication_init (i, (List.map compile_expression li))
      )::l
    | _ -> l
  in 
  ...
  Ast_helper.( 
    ...
    | IApplication (id, el) ->
      let listexp = match el with
        ...
        | _ ->
          [("",
            Exp.tuple (List.map (fun x -> [%expr [%e tocaml_expression x ]]) el)
           )] in 
      Exp.apply
    ...
    |IApplication_init (id, el) ->
      let listexp = match el with
        ...
        | _ -> [("",
                 Exp.tuple (List.map
                              (fun x ->[%expr [%e tocaml_expression x ]])
                              el)
                )] in 
      Exp.apply
        (Exp.ident (ident_to_lid id)) listexp
