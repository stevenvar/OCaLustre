...
type clock = 
  Global | Clock of string | Constant | No
...
type c_node = {
 c_name : ident ;
 c_inputs : (ident * string) list;  
 c_outputs : (ident *string) list;
...
let clock_pattern p c env = 
  match p with 
  | Simple i ->
    let cc = clock_to_str c in 
    let ck = Clocks.find env i.content in
    if ck = No || c = ck then 
      C_Simple (i,c), Clocks.add env i.content (c)
    else failwith ("wrong clock for declaration of pattern "^i.content^":"^clock_to_str ck^"<>"^cc) 
  | List il ->
    List.iter
      (fun i ->
         let cc = clock_to_str c in 
         ...
         if ck <> No && c <> ck then
           failwith
             ("wrong clock for declaration of pattern "^
              i.content^":"^
              clock_to_str ck^"<>"^
              cc) ) il ;          
    C_List (List.map (fun i -> i, c) il),
    Clocks.adds env (List.map (fun i -> i.content, c) il)
...
let mk_clock s =
  if s = "base" then Global else Clock s

let clock_node n = 
  let cin = List.map (fun (i,c) -> (i.content, mk_clock c)) n.inputs in
  let cout = List.map (fun (i,c) -> (i.content, mk_clock c)) n.outputs in
