...
type node = {
 name : ident;
 inputs : (ident * string) list;  
 outputs : (ident * string) list;
      ...
      | Pstr_eval (expression, _)  ->
        begin  match expression.pexp_desc with 
          | Pexp_ident i -> begin
              match i.txt with
              |  Lident s -> s
              | _ -> failwith "getCk failed"
            end
          | _ -> failwith "getCk failed" end 
      |_ -> failwith "getCk failed"
    end 
  |_ -> failwith "getCk failed"

let rec read_attributes attrs =
  match attrs with
  | (string_loc,payload)::tl ->
    (getCk payload)::(read_attributes tl)
  | [] -> ["base"]
...
let checkio body =
  ...
  | [%expr fun [%p? inputs] -> [%e? body] ] ->
    begin match inputs.ppat_desc with
      | Ppat_var s ->
        begin
          match read_attributes inputs.ppat_attributes with
          | [] -> ([checkname_pattern inputs, "base"], body)
          | h::t -> ([checkname_pattern inputs, h],body)
        end
      | Ppat_tuple l ->
        let clocks = List.map (fun i -> read_attributes i.ppat_attributes) l in  
        begin match clocks with
          | [] -> let tuples =
                    List.map (fun pat -> (pat,"base"))
                      (List.map checkname_pattern l)
            in tuples, body
          | h::t -> let tuples =
                      List.map2 (fun pat cl -> (pat,cl))
                        (List.map checkname_pattern l)
                        (h)
            in tuples,body
        end
